...
section "PHP 8.2: Fix LiveUser::factory()"
...
for filename in sys.argv[1:]:
    ...
    if re.search(r'\$this\b', body):
        print(f"REFUSED: LiveUser::{METHOD}() uses $this in {path}")
        print()
        print(body[:6000])
        raise SystemExit(
            f"LiveUser::{METHOD}() cannot safely be static."
        )
    ...
    if count != 1:
        raise SystemExit(
            f"Could not rewrite LiveUser::{METHOD}() in {path}"
        )

    backup = path.with_name(
        path.name + ".before-php82-liveuser-factory-static"
    )
...
section "LiveUser factory and singleton declarations"

echo "--- Source ---"

grep -n -A35 -B10 \
    -E '(factory|singleton)[[:space:]]*\(' \
    "$SOURCE" \
    | head -n 240
...
docker compose \
    ...
    php -d include_path=/var/www/html/ch/lib/pear -r '
        ...
        foreach (array("singleton", "factory") as $name) {
...
section "Static calls inside LiveUser"

grep -nE \
    'LiveUser::[A-Za-z_][A-Za-z0-9_]*[[:space:]]*\(' \
    "$SOURCE" \
    | head -n 200 \
    || true
