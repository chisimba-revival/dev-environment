...
section "PHP 8.2: Fix LiveUser::authFactory()"
...
for filename in sys.argv[1:]:
    ...
    backup = path.with_name(
        path.name + ".before-php82-authfactory-static"
    )
...
echo 'LiveUser::authFactory static: ',
...
section "Other LiveUser static calls"

grep -RIn \
    --include='*.php' \
    --include='*.inc' \
    --include='*.inc.php' \
    -E 'LiveUser::[A-Za-z_][A-Za-z0-9_]*[[:space:]]*\(' \
