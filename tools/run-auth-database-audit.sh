...
PYTHON="$TOOLS/audit-auth-database-coupling.py"
OUT="$ROOT/killme.txt"

TEXT="$REPORTS/auth-database-coupling-audit.txt"
MARKDOWN="$REPORTS/auth-database-coupling-audit.md"
JSON="$REPORTS/auth-database-coupling-audit.json"
...
rm -f "$TEXT" "$MARKDOWN" "$JSON"

{
    echo "============================================================"
    echo "AUTHENTICATION DATABASE COUPLING AUDIT"
    echo "============================================================"
    echo
    echo "Started: $(date --iso-8601=seconds)"
    echo

    timeout 180 python3 "$PYTHON" \
        --root "$ROOT" \
        --text "$TEXT" \
        --markdown "$MARKDOWN" \
        --json "$JSON"
    ...
    echo "  $JSON"
    echo
    echo "No application source files or database data were changed."
